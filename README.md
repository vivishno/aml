...
Arm Template is present in ./cloud/.azure/ folder (deafault name=arm_deploy.json) is used to deploy azure resources to azure . It uses the parameters provided in file azure.params.json to create new resources or update the resources if they are already present.
# Documentation
...
| `GitHubBranch`  | Name of the branch to use when deploying (Default = master). |
...
| `GitHubURL`           | The URL of GitHub (ending by .git) |
...
| `pat_token`                        | pat token to be used by the function app to communicate to github. |
